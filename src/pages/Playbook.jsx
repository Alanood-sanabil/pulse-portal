...
import { Search, BookOpen, Bookmark, BookmarkCheck, Share2, Clock, Sparkles } from 'lucide-react'
...
Managing the agency relationship well is the part most founders underestimate. Assign one internal person as the agency owner. Set a monthly check-in with a written agenda. Require a media coverage tracker updated weekly.`,
  },
  {
    ...
    readTime: 9,
    content: `A term sheet is a non-binding document that outlines the key economic and governance terms of an investment. Most founders sign them too quickly, overwhelmed by the excitement of a commitment and underestimating how much the details matter when things go wrong — or right.
...
export default function Playbook() {
  ...
  // Recommend articles in topics not yet read
        ...
        {openArticle && (
          <div className="px-6 py-6 space-y-4">
            <div className="flex items-center justify-between">
              <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${TOPIC_COLORS[openArticle.topic] || 'bg-bg-elevated text-text-muted'}`}>
                {openArticle.topic}
              </span>
              <div className="flex items-center gap-2">
                <span className="flex items-center gap-1 text-xs text-text-dim">
                  <Clock size={11} /> {openArticle.readTime} min read
                </span>
                <button
                  onClick={e => handleShare(openArticle, e)}
                ...
                <button
                  onClick={e => toggleBookmark(openArticle.id, e)}
                  className={`w-7 h-7 flex items-center justify-center rounded-lg transition-colors ${bookmarks.has(openArticle.id) ? 'text-amber' : 'text-text-dim hover:text-amber hover:bg-bg-elevated'}`}
                  aria-label="Bookmark"
                >
                  {bookmarks.has(openArticle.id) ? <BookmarkCheck size={13} /> : <Bookmark size={13} />}
                </button>
              </div>
            </div>
            <div className="space-y-4">
              {openArticle.content.split('\n\n').map((paragraph, i) => (
                <p key={i} className="text-sm text-text-muted leading-relaxed">{paragraph}</p>
              ))}
            </div>
          </div>
        )}
